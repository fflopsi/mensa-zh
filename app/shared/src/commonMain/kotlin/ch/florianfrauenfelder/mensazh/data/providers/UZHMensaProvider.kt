...
import ch.florianfrauenfelder.mensazh.BuildKonfig
...
class UZHMensaProvider(menuDao: MenuDao, fetchInfoDao: FetchInfoDao) :
  ...
  override fun HttpRequestBuilder.request(destination: Destination, language: Language) {
    ...
    headers {
      ...
      append("api-key", BuildKonfig.ZFV_API_KEY)
    }
  ...
  private fun parseApiMenu(
    ...
    if (menuItem.dish == null || menuItem.category?.name == null || i18nName.label == null) null
      ...
      price = menuItem.prices?.mapNotNull { it.amount?.toDouble() }?.sorted()
        ?.map { it.formatPrice() } ?: emptyList(),
